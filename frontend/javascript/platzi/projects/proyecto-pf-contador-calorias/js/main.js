...
const tagAttrs =
  (obj) =>
  (content = "") =>
    `<${obj.tag}${obj.attrs ? " " : ""}${attrToString(
      obj.attrs
    )}> ${content} </${obj.tag}>`;
...
const editItem = (index) => {
  ...
  if ($buttonMOD.getAttribute("data-status") == "start") {
    Array.prototype.map.call($rowToEdit, (item) => {
      item.disabled = false;
...
$DESC.addEventListener("keyup", () => {
  $DESC.classList.remove(IS_INVALID);
});
...
$PROTEIN.addEventListener("keyup", () => {
  $PROTEIN.classList.remove(IS_INVALID);
});
...
(() => {
  ...
  console.log(items);
  items.length === 0 ? 0 : (itemsList = items), updateTotal(), renderItems();
})();
